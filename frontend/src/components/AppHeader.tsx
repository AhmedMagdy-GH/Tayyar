    ...
    <div className="shell app-header__inner">
      ...
      <nav className="header-actions" aria-label="Account shortcuts">
        {session.isPending ? <span className="header-session-loading" role="status">Checking session…</span> : session.isError ? <button className="header-session-error" type="button" onClick={() => void session.refetch()}>Session unavailable · retry</button> : authenticated ? <>
          <Link className="icon-button header-actions__secondary" to="/favorites" aria-label="Favorites" title="Favorites"><Heart size={19} /></Link>
      ...
      {menuOpen && <div className="mobile-account-menu" role="menu">{authenticated ? <><div><strong>{session.data?.fullName}</strong><span>{session.data?.email}</span></div><Link role="menuitem" to="/account"><UserRound size={16} /> Account</Link><Link role="menuitem" to="/orders"><ShoppingBag size={16} /> Orders</Link><Link role="menuitem" to="/favorites"><Heart size={16} /> Favorites</Link><Link role="menuitem" to="/notifications"><Bell size={16} /> Notifications{unread.data?.total ? ` (${unread.data.total})` : ''}</Link><Link role="menuitem" to="/cart"><ShoppingBag size={16} /> Cart{cartCount ? ` (${cartCount})` : ''}</Link><Link role="menuitem" to="/addresses"><MapPin size={16} /> Saved addresses</Link><button role="menuitem" type="button" onClick={() => logout.mutate()} disabled={logout.isPending}><LogOut size={16} /> {logout.isPending ? 'Signing out…' : 'Sign out'}</button></> : <><Link role="menuitem" to="/login" state={{ from: returnPath }}>Log in</Link><Link role="menuitem" to="/register" state={{ from: returnPath }}>Create account</Link></>}</div>}
    </div>
