  ...
  return <div className="page-shell"><AppHeader /><main className="shell customer-page">
    ...
    {cart && <div className="cart-layout">
      <section className="cart-items" aria-labelledby="cart-branch"><header><div><small>Ordering from</small><h2 id="cart-branch">{cart.branch.restaurantName}</h2><p>{cart.branch.name} branch</p></div><span className={cart.branch.openNow ? 'status-open' : 'status-closed'}>{cart.branch.openNow ? 'Open now' : 'Closed'}</span></header>
        ...
        <div className="cart-lines">{cart.items.map((line) => <article className="cart-line" key={line.id}><div className="cart-line__art" aria-hidden="true">🍽️</div><div className="cart-line__copy"><h3>{line.name}</h3>{line.priceChanged ? <p><del>{money(line.acknowledgedUnitPrice, cart.currency)}</del> <strong>{money(line.currentUnitPrice, cart.currency)}</strong> each</p> : <p>{money(line.currentUnitPrice, cart.currency)} each</p>}{!line.currentlyAvailable && <span>Currently unavailable</span>}</div><div className="cart-line__actions"><div className="quantity-control quantity-control--static"><button type="button" disabled={update.isPending} onClick={() => update.mutate({ lineId: line.id, quantity: line.quantity - 1, cart, version: line.version })} aria-label={line.quantity === 1 ? `Remove ${line.name}` : `Decrease ${line.name} quantity`}><Minus size={15} /></button><strong>{line.quantity}</strong><button type="button" disabled={update.isPending || line.quantity >= 99} onClick={() => update.mutate({ lineId: line.id, quantity: line.quantity + 1, cart, version: line.version })} aria-label={`Increase ${line.name} quantity`}><Plus size={15} /></button></div><strong>{money(line.lineSubtotal, cart.currency)}</strong></div></article>)}</div>
      </section>
      <aside className="cart-summary"><h2>Cart summary</h2><div><span>Items subtotal</span><strong>{money(cart.merchandiseSubtotal, cart.currency)}</strong></div>{priceChanged && <button className="reconfirm-button" type="button" disabled={reconfirm.isPending || unavailable} onClick={() => reconfirm.mutate()}><RefreshCw size={16} /> {reconfirm.isPending ? 'Confirming…' : 'Confirm current prices'}</button>}{!priceChanged && <p className="prices-current"><CheckCircle2 size={16} /> Prices are current</p>}<button className="checkout-disabled" type="button" disabled>Checkout comes in the next phase</button><small>Delivery fees and promotions are calculated during checkout.</small></aside>
    </div>}
