                        ...
                        status -> {
                            ...
                            jdbc.update(
                                    "INSERT INTO"
                                        ...
                                        + " VALUES (?,?,'Snapshot"
                                        + " Kitchen','Original','ACTIVE',now(),now())",
                                    restaurant,
                                    application);
                            jdbc.update(
...
""",
    ...
    void itemAndAddressSnapshotsSurviveMutableSourceChangesAndAddressDeletion() {
        ...
        jdbc.update(
                "UPDATE customer_addresses SET street='New Street',building='99' WHERE id=?",
                savedAddress);
    ...
    void branchRestaurantContextAndMoneyAreValidatedInApplicationAndDatabase() {
        ...
        sqlState(
                ...
                () ->
                        jdbc.update(
                                "INSERT INTO"
                                    + " orders(id,customer_id,restaurant_id,branch_id,status,currency,"
    ...
    void orderLifecycleHistoryAndTerminalRulesHold() {
        ...
        for (OrderStatus status :
            ...
            order =
                    orders.transition(
                            order.id(), order.version(), status, TransitionActor.system(), null);
    ...
    void concurrentOrderTransitionsHaveOneWinnerAndOneConflict() throws Exception {
        ...
        Callable<String> cancel =
                transition(order, OrderStatus.CANCELLED, "cancelled", ready, start);
    ...
    Callable<String> transition(
        ...
        return () -> {
            ...
            try {
                orders.transition(
                        order.id(), order.version(), target, TransitionActor.system(), reason);
                return "SUCCESS";
    ...
    void cardRulesExistWithoutFakeProviderProcessing() {
        ...
        assertThat(orders.details(order.id()).order().status())
    ...
    void historicalRowsRejectMutationDeletionAndDestructiveCascades() {
        ...
        List<org.assertj.core.api.ThrowableAssert.ThrowingCallable> protectedMutations =
                List.of(
                        ...
                        () ->
                                jdbc.update(
                                        "UPDATE order_items SET purchased_name='Changed' WHERE"
                                                + " id=?",
                                        orderItem),
                        () ->
                                jdbc.update(
                                        "DELETE FROM order_address_snapshots WHERE order_id=?",
                                        order),
    ...
    @Test
    void databaseContextConstraintsIndexesNoCardSecretsAndNoPaymentApiExist() {
        Details details = create(OrderStatus.PLACED);
        ...
        assertThat(
                        mappings.getHandlerMethods().values().stream()
                                ...
                                .noneMatch(name -> name.equals("com.tayyar.payment")))
                .isTrue();
    }
    ...
    void sqlState(String expected, org.assertj.core.api.ThrowableAssert.ThrowingCallable work) {
        assertThatThrownBy(work)
                ...
                .isInstanceOfSatisfying(
                        SQLException.class,
                        error -> assertThat(error.getSQLState()).isEqualTo(expected));
    }
}
